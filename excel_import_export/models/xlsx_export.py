...
class XLSXExport(models.AbstractModel):
    ...
    def _get_line_vals(self, record, line_field, fields):
        ...
        print('_get_line_vals_before ===', record)
        #print('_get_line_vals_before ===', record.__str__)

        #print('_get_line_vals_before ===', record[line_field])

        #if record.get_report_model_name() and record.get_report_model_name()=='stock.pos.order.wizard' :
        #    lines=record.results
        #else :
        #    lines = record[line_field]
        lines = record[line_field]
        print('_get_line_vals_before ===', lines)
        if max_row > 0 and len(lines) > max_row:
            raise Exception(_("Records in %s exceed max records allowed") % line_field)
        print('_get_line_vals ===', fields)
        ...
        print('_get_line_vals ===', lines)
        print('_get_line_vals ===', pair_fields)

        for line in lines:
            for field in pair_fields:  # (field, raw_field)
                print('_get_field_line ===', line)
                print('_get_field_data_field ===', field)
                value = self._get_field_data(field[1], line)
                print('_field_data ===', field[1])
    ...
    def _get_field_data(self, _field, _line):
        ...
        if not _field:
            return None
        line_copy = _line
        print('_get_field_data_line ===', line_copy)
