...
*/
    ...
    PivotController.include({
        ...
        generatePDF2: function() {
            var form = $('.o_pivot').html();
            var title = $('ol.breadcrumb').find('li.active').html();
            var style = $('head').html();
            var stylelink= $('[data-asset-xmlid="web.assets_backend"]').attr('href');
            var iframe= document.createElement('iframe');
            $('body').append(iframe);
            iframe.setAttribute("style","height:100%;width:100%;");
            iframe.setAttribute("name","myiframe");
            iframe.setAttribute("class","printFrame1");
            iframe.contentWindow.document.open();
            iframe.contentWindow.document.write('<html><head><link type="text/css" rel="stylesheet" href="'+stylelink+'" data-asset-xmlid="web.assets_common" ><title>'+title+'</title></head>');
            iframe.contentWindow.document.write(form);
            iframe.contentWindow.document.write('</body></html>');
            iframe.contentWindow.document.close();

            function createPDF() {
                setTimeout(() => { window.frames['myiframe'].focus(); }, 2000);
                setTimeout(() => { window.frames['myiframe'].print(); }, 3000);


                 setTimeout(() => { $('.printFrame1').remove(); }, 5000);
            }
            $('body').scrollTop(0);
            createPDF();
        },
