...
class PurchaseOrderReportAnalyis(models.Model):
    ...
    date_order = fields.Datetime('تاريخ الطلب', readonly=True, help="Depicts the date when the Quotation should be validated and converted into a purchase order.")
    state = fields.Selection([
        ...
        ('done', 'Done'),
        ('cancel', 'Cancelled')
    ], 'حالة الطلب', readonly=True)
    product_id = fields.Many2one('product.product', 'المنتج', readonly=True)
    partner_id = fields.Many2one('res.partner', 'المورد', readonly=True)
    date_approve = fields.Datetime('تاريخ التأكيد', readonly=True)
    ...
    currency_id = fields.Many2one('res.currency', 'العملة', readonly=True)
    user_id = fields.Many2one('res.users', 'مسوؤل المبيعات', readonly=True)
    delay = fields.Float('الايام للتأكيد', digits=(16, 2), readonly=True, help="Amount of time between purchase approval and order by date.")
    delay_pass = fields.Float('الايام للاستلام', digits=(16, 2), readonly=True, help="Amount of time between date planned and order by date for each purchase order line.")
    avg_days_to_purchase = fields.Float(
        'متوسط الايام للشراء', digits=(16, 2), readonly=True, store=False,  # needs store=False to prevent showing up as a 'measure' option
    ...
    price_total = fields.Float('الاجمالي', readonly=True)
    price_average = fields.Float('متوسط السعر', readonly=True, group_operator="avg")
    nbr_lines = fields.Integer('اجمالي عدد المنتجات في الطلب', readonly=True)
    category_id = fields.Many2one('product.category', 'فئة المنتج', readonly=True)
    product_tmpl_id = fields.Many2one('product.template', 'قالب المنتج', readonly=True)
    country_id = fields.Many2one('res.country', 'الدولة', readonly=True)
    fiscal_position_id = fields.Many2one('account.fiscal.position', string='Fiscal Position', readonly=True)
    account_analytic_id = fields.Many2one('account.analytic.account', 'Analytic Account', readonly=True)
    commercial_partner_id = fields.Many2one('res.partner', 'المورد', readonly=True)
    #weight = fields.Float('Gross Weight', readonly=True)
    #volume = fields.Float('Volume', readonly=True)
    order_id = fields.Many2one('purchase.order', 'Order', readonly=True)
    untaxed_total = fields.Float('الاجمالي قبل الضريبة', readonly=True)
    qty_ordered = fields.Float('الكمية المطلوبة', readonly=True)
    qty_received = fields.Float('الكمية المستلمة', readonly=True)
    qty_billed = fields.Float('الكمية المفوتره', readonly=True)
    qty_to_be_billed = fields.Float('الكمية القابلة للفوتره', readonly=True)
    ...
    def read_group(self, domain, fields, groupby, offset=0, limit=None, orderby=False, lazy=True):
        ...
        res = []
        if fields:
            res = super(PurchaseOrderReportAnalyis, self).read_group(domain, fields, groupby, offset=offset, limit=limit, orderby=orderby, lazy=lazy)
